...
{-------------------------------------------------------------------------------
  ...
  License
    ...
    As part of IM.units.lpk, this file is distributed under the terms
...
Type
  ...
  Private
    ...
    Procedure FiLambdaH2XYZ(Fi, Lambda, H: Extended; Var x, y, z: Extended; system: String);
    Procedure XYZ2FiLambdaH(x, y, z: Extended; Var Fi, Lambda, H: Extended; system: String);

    Procedure PlanaXY2FiLambdaH(x, y: Extended; Var Fi, Lambda, H: Extended;
      system, medelMeridian: String);
    Procedure FiLambdaH2PlanaXY(Fi, Lambda, H: Extended; Var x, y: Extended;
      system, medelMeridian: String);
    ...
    Procedure FiLambdaH2PlanaXYSWEREF99(Fi, Lambda, H: Extended;
      Var x, y: Extended; system: String);
    Procedure PlanaXY2FiLambdaHSWEREF99(x, y: Extended; Var Fi, Lambda, H: Extended;
      system: String);
  Public
    { Public declarations }
    Constructor Create;
    ...
    Procedure WGS84ToSWEREF99(Fi, Lambda, H: Extended; Var x, y: Extended);
    Procedure WGS84AsDegreeAndDecimalMinuteToSWEREF99(Fi, Lambda, H: Extended;
      Var x, y: Extended);
    Procedure SWEREF99ToWGS84(x, y: Extended; Var Fi, Lambda, H: Extended);
    Procedure SWEREF99ToWGS84AsDegreeAndDecimalMinute(x, y: Extended;
      Var Fi, Lambda, H: Extended);
    Procedure RT90ToSWEREF99(xRT90, yRT90: Extended; Var xSWEREF99, ySWEREF99: Extended);
    Procedure SWEREF99ToRT90(xSWEREF99, ySWEREF99: Extended; Var xRT90, yRT90: Extended);
    Procedure GPSToRT90(Fi, Lambda, H: Extended; medelMeridian: String; Var x, y: Extended);
    Procedure GPSAsDegreeAndDecimalMinuteToRT90(Fi, Lambda, H: Extended;
      medelMeridian: String; Var x, y: Extended);
    Procedure RT90ToGPS(x, y: Extended; medelMeridian: String; Var Fi, Lambda, H: Extended);
    Procedure RT90ToGPSAsDegreeAndDecimalMinute(x, y: Extended; medelMeridian: String;
      Var Fi, Lambda, H: Extended);
    Procedure WGS84ToUTM(LatLon: TrecLatLon; Var UTM: TrecUTM);
    Procedure UTMToWGS84(UTM: TrecUTM; Var latlon: TrecLatLon);

    Property IgnoreGeodeticOrientation: Boolean Read FIgnoreGeodeticOrientation
      Write FIgnoreGeodeticOrientation;
  End;

Var
  cosXToRR92, sinXToRR92, cosYToRR92, sinYToRR92, cosZToRR92, sinZToRR92: Extended;
  cosXToSWEREF93, sinXToSWEREF93, cosYToSWEREF93, sinYToSWEREF93, cosZToSWEREF93,
  sinZToSWEREF93: Extended;
  a1RR92, a2RR92, a3RR92, b1RR92, b2RR92, b3RR92, c1RR92, c2RR92, c3RR92: Extended;
  a1SWEREF93, a2SWEREF93, a3SWEREF93, b1SWEREF93, b2SWEREF93, b3SWEREF93,
  c1SWEREF93, c2SWEREF93, c3SWEREF93: Extended;
...
Begin
  ...
  cosXToRR92 := cos(XwToRR92);
  sinXToRR92 := sin(XwToRR92);
  cosYToRR92 := cos(YwToRR92);
  sinYToRR92 := sin(YwToRR92);
  cosZToRR92 := cos(ZwToRR92);
  sinZToRR92 := sin(ZwToRR92);
  ...
  cosXToSWEREF93 := cos(XwToSWEREF93);
  sinXToSWEREF93 := sin(XwToSWEREF93);
  cosYToSWEREF93 := cos(YwToSWEREF93);
  sinYToSWEREF93 := sin(YwToSWEREF93);
  cosZToSWEREF93 := cos(ZwToSWEREF93);
  sinZToSWEREF93 := sin(ZwToSWEREF93);
  ...
  a3SWEREF93 := -cosXToSWEREF93 * sinYToSWEREF93 * cosZToSWEREF93 + sinXToSWEREF93 *
    sinZToSWEREF93;
  ...
  Medelmeridianer[1].radianer :=
    GradMinutSekund2Radianer(Medelmeridianer[1].grad, Medelmeridianer[1].minut,
  ...
  Medelmeridianer[2].radianer :=
    GradMinutSekund2Radianer(Medelmeridianer[2].grad, Medelmeridianer[2].minut,
  ...
  Medelmeridianer[3].radianer :=
    GradMinutSekund2Radianer(Medelmeridianer[3].grad, Medelmeridianer[3].minut,
  ...
  Medelmeridianer[4].radianer :=
    GradMinutSekund2Radianer(Medelmeridianer[4].grad, Medelmeridianer[4].minut,
  ...
  Medelmeridianer[5].radianer :=
    GradMinutSekund2Radianer(Medelmeridianer[5].grad, Medelmeridianer[5].minut,
  ...
  Medelmeridianer[6].radianer :=
    GradMinutSekund2Radianer(Medelmeridianer[6].grad, Medelmeridianer[6].minut,
    Medelmeridianer[6].sekund);

  FIgnoreGeodeticOrientation := False;
End;

Procedure TWGS84.WGS84ToSWEREF99(Fi, Lambda, H: Extended; Var x, y: Extended);
Begin
  FiLambdaH2PlanaXYSWEREF99(Fi, Lambda, H, x, y, 'SWEREF99');
End;

Procedure TWGS84.SWEREF99ToWGS84(x, y: Extended; Var Fi, Lambda, H: Extended);
Begin
  PlanaXY2FiLambdaHSWEREF99(x, y, Fi, Lambda, H, 'SWEREF99');
End;

Procedure TWGS84.RT90ToSWEREF99(xRT90, yRT90: Extended; Var xSWEREF99, ySWEREF99: Extended);
Var
...
Procedure TWGS84.SWEREF99ToRT90(xSWEREF99, ySWEREF99: Extended; Var xRT90, yRT90: Extended);
Var
  Fi, Lambda, H: Extended;
Begin
...
Procedure TWGS84.GPSToRT90(Fi, Lambda, H: Extended; medelMeridian: String; Var x, y: Extended);
Begin
...
Procedure TWGS84.RT90ToGPS(x, y: Extended; medelMeridian: String; Var Fi, Lambda, H: Extended);
Begin
  If (medelMeridian = '') Or (medelMeridian = ' ') Then
    medelMeridian := '2.5 gon V';
...
Procedure TWGS84.SWEREF93ToRR92xyz(xSWEREF93, ySWEREF93, zSWEREF93: Extended;
  Var xRR92, yRR92, zRR92: Extended);
Begin
  xRR92 := (1 + skalfToRR92) * (a1RR92 * xSWEREF93 + a2RR92 * ySWEREF93 + a3RR92 * zSWEREF93) +
    XTranslToRR92;
  yRR92 := (1 + skalfToRR92) * (b1RR92 * xSWEREF93 + b2RR92 * ySWEREF93 + b3RR92 * zSWEREF93) +
    YTranslToRR92;
  zRR92 := (1 + skalfToRR92) * (c1RR92 * xSWEREF93 + c2RR92 * ySWEREF93 + c3RR92 * zSWEREF93) +
    ZTranslToRR92;
End;
...
Procedure TWGS84.RR92ToSWEREF93xyz(xRR92, yRR92, zRR92: Extended;
  Var xSWEREF93, ySWEREF93, zSWEREF93: Extended);
Begin
  xSWEREF93 := (1 + skalfToSWEREF93) * (a1SWEREF93 * xRR92 + a2SWEREF93 * yRR92 +
    a3SWEREF93 * zRR92) + XTranslToSWEREF93;
  ySWEREF93 := (1 + skalfToSWEREF93) * (b1SWEREF93 * xRR92 + b2SWEREF93 * yRR92 +
    b3SWEREF93 * zRR92) + YTranslToSWEREF93;
  zSWEREF93 := (1 + skalfToSWEREF93) * (c1SWEREF93 * xRR92 + c2SWEREF93 * yRR92 +
    c3SWEREF93 * zRR92) + ZTranslToSWEREF93;
End;

Procedure TWGS84.FiLambdaH2XYZ(Fi, Lambda, H: Extended; Var x, y, z: Extended; system: String);
Var
  sinFi: Extended;
Begin
  Fi := DegToRad(Fi);
...
Procedure TWGS84.XYZ2FiLambdaH(x, y, z: Extended; Var Fi, Lambda, H: Extended; system: String);
Var
...
Begin
  ...
  Begin
    ...
    tanFi := (z + ((halvaStoraxelnBessel * excentricitetKvadratenBessel) /
      sqrt(1 - excentricitetKvadratenBessel)) * sinTheta * sinTheta * sinTheta) /
      (p - halvaStoraxelnBessel * excentricitetKvadratenBessel * cosTheta *
      cosTheta * cosTheta);
    Fi := ArcTan(tanFi);
    sinFi := sin(Fi);
    TvarKrokRadieBessel := halvaStoraxelnBessel /
  ...
  Begin
    ...
    tanFi := (z + ((halvaStoraxelnSWEREF93 * excentricitetKvadratenSWEREF93) /
      sqrt(1 - excentricitetKvadratenSWEREF93)) * sinTheta * sinTheta * sinTheta) /
      (p - halvaStoraxelnSWEREF93 * excentricitetKvadratenSWEREF93 * cosTheta *
      cosTheta * cosTheta);
    Fi := ArcTan(tanFi);
    sinFi := sin(Fi);
    TvarKrokRadieSWEREF93 := halvaStoraxelnSWEREF93 /
...
Procedure TWGS84.PlanaXY2FiLambdaH(x, y: Extended; Var Fi, Lambda, H: Extended;
  system, medelMeridian: String);
Var
...
Begin
  Try
    ...
    Begin
      ...
      nKvadrat := n * n;
      aTak := (halvaStoraxelnBessel / (1 + n)) * (1 + (1 / 4) * nKvadrat +
        (1 / 64) * nKvadrat * nKvadrat);

      Ksi := x / (medelMeridianFaktorSWEREF93 * aTak);
      Aeta := y / (medelMeridianFaktorSWEREF93 * aTak);

      d1 := (1 / 2) * n - (2 / 3) * nKvadrat + (37 / 96) * n * nKvadrat -
        (1 / 360) * nKvadrat * nKvadrat;
      ...
      KsiPrim := Ksi - d1 * sin(2 * Ksi) * cosh(2 * Aeta) - d2 * sin(4 * Ksi) *
        cosh(4 * Aeta) - d3 * sin(6 * Ksi) * cosh(6 * Aeta) - d4 * sin(8 * Ksi) * cosh(8 * Aeta);
      AetaPrim := Aeta - d1 * cos(2 * Ksi) * sinh(2 * Aeta) - d2 * cos(4 * Ksi) *
        sinh(4 * Aeta) - d3 * cos(6 * Ksi) * sinh(6 * Aeta) - d4 * cos(8 * Ksi) * sinh(8 * Aeta);
      ...
      sinFiIso := sin(FiISo);
      Fi := FiIso + sinFiIso * cos(FiIso) * (A + B * sinFiIso * sinFiIso +
        C * sinFiIso * sinFiIso * sinFiIso * sinFiIso + D * sinFiIso * sinFiIso *
        sinFiIso * sinFiIso * sinFiIso * sinFiIso);
    ...
    Begin
      ...
      nKvadrat := n * n;
      aTak := (halvaStoraxelnSWEREF93 / (1 + n)) * (1 + (1 / 4) * nKvadrat +
        (1 / 64) * nKvadrat * nKvadrat);
...
Procedure TWGS84.FiLambdaH2PlanaXY(Fi, Lambda, H: Extended; Var x, y: Extended;
  system, medelMeridian: String);
Var
...
Begin
  ...
  Begin
    ...
    FiIso := Fi - sinFi * cos(Fi) * (A + B * sinFi * sinFi + C * sinFi * sinFi *
      sinFi * sinFi + D * sinFi * sinFi * sinFi * sinFi * sinFi * sinFi);

    KsiPrim := ArcTan(tan(FiIso) / cos(deltaLambda));
    AetaPrim := ArcTanh(cos(FiIso) * sin(deltaLambda));

    b1 := (1 / 2) * n - (2 / 3) * nKvadrat + (5 / 16) * n * nKvadrat + (41 / 180) *
      nKvadrat * nKvadrat;
    b2 := (13 / 48) * nKvadrat - (3 / 5) * n * nKvadrat + (557 / 1440) * nKvadrat * nKvadrat;
    b3 := (61 / 240) * n * nKvadrat - (103 / 140) * nKvadrat * nKvadrat;
    b4 := (49561 / 161280) * nKvadrat * nKvadrat;

    x := cosh(2 * AetaPrim);
    x := medelMeridianFaktorSWEREF93 * aTak * (KsiPrim + b1 * sin(2 * KsiPrim) *
      cosh(2 * AetaPrim) + b2 * sin(4 * KsiPrim) * cosh(4 * AetaPrim) + b3 *
      sin(6 * KsiPrim) * cosh(6 * AetaPrim) + b4 * sin(8 * KsiPrim) * cosh(8 * AetaPrim));
    y := medelMeridianFaktorSWEREF93 * aTak * (AetaPrim + b1 * cos(2 * KsiPrim) *
      sinh(2 * AetaPrim) + b2 * cos(4 * KsiPrim) * sinh(4 * AetaPrim) + b3 *
      cos(6 * KsiPrim) * sinh(6 * AetaPrim) + b4 * cos(8 * KsiPrim) * sinh(8 * AetaPrim));
  End
  Else
  Begin
    sinFi := sin(Fi);
...
Procedure TWGS84.PlanaXY2FiLambdaHSWEREF99(x, y: Extended; Var Fi, Lambda, H: Extended;
  system: String);
Var
  n, nKvadrat, aTak, Meridian, deltaLambda: Extended;
  Aeta, Ksi, AetaPrim, KsiPrim: Extended;
...
Begin
  Try
    ...
    nKvadrat := n * n;
    aTak := (halvaStoraxeln / (1 + n)) * (1 + (1 / 4) * nKvadrat + (1 / 64) * nKvadrat * nKvadrat);

    Ksi := x / (skalFaktor * aTak);
    Aeta := y / (skalFaktor * aTak);

    d1 := (1 / 2) * n - (2 / 3) * nKvadrat + (37 / 96) * n * nKvadrat - (1 / 360) *
      nKvadrat * nKvadrat;
    d2 := (1 / 48) * nKvadrat + (1 / 15) * n * nKvadrat - (437 / 1440) * nKvadrat * nKvadrat;
    d3 := (17 / 480) * n * nKvadrat - (37 / 840) * nKvadrat * nKvadrat;
    d4 := (4397 / 161280) * nKvadrat * nKvadrat;
    ...
    sinFiIso := sin(FiISo);
    Fi := FiIso + sinFiIso * cos(FiIso) * (A + B * sinFiIso * sinFiIso + C *
      sinFiIso * sinFiIso * sinFiIso * sinFiIso + D * sinFiIso * sinFiIso * sinFiIso *
      sinFiIso * sinFiIso * sinFiIso);
...
Begin
  Try
    ...
    x := skalFaktor * aTak * (KsiPrim + b1 * sin(2 * KsiPrim) * cosh(2 * AetaPrim) +
      b2 * sin(4 * KsiPrim) * cosh(4 * AetaPrim) + b3 * sin(6 * KsiPrim) *
      cosh(6 * AetaPrim) + b4 * sin(8 * KsiPrim) * cosh(8 * AetaPrim));
    y := skalFaktor * aTak * (AetaPrim + b1 * cos(2 * KsiPrim) * sinh(2 * AetaPrim) +
      b2 * cos(4 * KsiPrim) * sinh(4 * AetaPrim) + b3 * cos(6 * KsiPrim) *
      sinh(6 * AetaPrim) + b4 * cos(8 * KsiPrim) * sinh(8 * AetaPrim));
...
Procedure TWGS84.WGS84AsDegreeAndDecimalMinuteToSWEREF99(Fi, Lambda, H: Extended;
  Var x, y: Extended);
Var
  minute: Extended;
  strMinute: String;
...
Var
  N, nu2, ep2, t, t2, l, phi, lambda: Double;
Var
  l3coef, l4coef, l5coef, l6coef, l7coef, l8coef: Double;
Begin
  phi := LatLon.Lat;
  lambda := LatLon.Lon;
  // Precalculate ep2
  ...
  // Calculate easting (x)
  UTM.X := N * cos(phi) * l + (N / 6 * intPower(cos(phi), 3) * l3coef * intPower(l, 3)) +
    (N / 120 * intPower(cos(phi), 5) * l5coef * intPower(l, 5)) +
    (N / 5040 * intPower(cos(phi), 7) * l7coef * intPower(l, 7));

  // Calculate northing (y)
  UTM.Y := ArcLengthOfMeridian(phi) + (t / 2 * N * intPower(cos(phi), 2) * intPower(l, 2)) +
...
Var
  phif, Nf, Nfpow, nuf2, ep2, tf, tf2, tf4, cf, x, y: Double;
Var
  x1frac, x2frac, x3frac, x4frac, x5frac, x6frac, x7frac, x8frac: Double;
Var
...
Begin
  ...
  x1frac := 1 / (Nfpow * cf);
  Nfpow := Nfpow * Nf;
  x2frac := tf / (2 * Nfpow);  // now equals Nf**2)
  Nfpow := Nfpow * Nf;
  x3frac := 1 / (6 * Nfpow * cf);  // now equals Nf**3)
  Nfpow := Nfpow * Nf;
  x4frac := tf / (24 * Nfpow);  // now equals Nf**4)
  Nfpow := Nfpow * Nf;
  x5frac := 1 / (120 * Nfpow * cf);  // now equals Nf**5)
  Nfpow := Nfpow * Nf;
  x6frac := tf / (720 * Nfpow);  // now equals Nf**6)
  Nfpow := Nfpow * Nf;
  x7frac := 1 / (5040 * Nfpow * cf);  // now equals Nf**7)
  Nfpow := Nfpow * Nf;
  x8frac := tf / (40320 * Nfpow);  // now equals Nf**8)
  // Precalculate polynomial coefficients for x**n.
  // -- x**1 does not have a polynomial coefficient.
  x2poly := -1 - nuf2;
  x3poly := -1 - 2 * tf2 - nuf2;
  x4poly := 5 + 3 * tf2 + 6 * nuf2 - 6 * tf2 * nuf2 - 3 * (nuf2 * nuf2) - 9 *
    tf2 * (nuf2 * nuf2);
  ...
  // Calculate latitude
  philambda.Lat := phif + x2frac * x2poly * (x * x) + x4frac * x4poly *
    intPower(x, 4) + x6frac * x6poly * intPower(x, 6) + x8frac * x8poly * intPower(x, 8);
